...
const Load_comp = () => {
  ...
  // Extract amazon_url and flipkart_url from state (passed from Sum.jsx)
  ...
  useEffect(() => {
    ...
    const timeoutId = setInterval(() => {
      ...
      if (elapsedTime >= maxWaitTime) {
        setError('Failed to fetch data within 60 seconds.');
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('http://127.0.0.1:5000/', {
          ...
          body: JSON.stringify({ amazon_url, flipkart_url }),
        });
        ...
        if (data) {
          ...
          navigate('/compres', { state: { result: JSON.stringify(data) } }); // Navigate with the result
        } else {
  ...
  }, [amazon_url, flipkart_url, navigate]);
